...
describe("settingsStore", () => {
  it("load fetches settings", async () => {
    const settings = { confirm_before_delete: true, default_folder_id: null, theme: "dark" as const, font_size: "medium" as const, default_ports: { postgresql: 5432, mysql: 3306, redis: 6379, sqlite: null }, tag_order: null };
  ...
  it("updateSetting persists then reloads", async () => {
    ...
    const settings = { confirm_before_delete: true, default_folder_id: null, theme: "light" as const, font_size: "medium" as const, default_ports: { postgresql: 5432, mysql: 3306, redis: 6379, sqlite: null }, tag_order: null };
    vi.spyOn(commands, "getSettings").mockResolvedValue(settings);
