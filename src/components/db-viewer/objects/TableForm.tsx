...
const SqlEditorField = lazy(() =>
    import("../../editor/SqlEditorField").then((m) => ({
        default: m.SqlEditorField,
    })),
);
...
// Select styling matching the FK panel (transparent, no surface bg).
const selectTransparent =
    "min-w-0 flex-1 bg-transparent font-heading text-xs text-text outline-none placeholder:text-text-muted cursor-pointer";
...
export function TableForm({
    ...
    const liveTab = useDbViewerStore(
        (s) =>
            s.tabs.find((t) => t.id === tab.id && t.tabType === "objectForm") ??
            tab,
    );
    ...
    const [fkPanel, setFkPanel] = useState<{
        open: boolean;
        column: string | null;
    } | null>(null);
    ...
    useEffect(() => {
        ...
        let active = true;
        cmd.getTableRebuildReadiness(connectionId, params.schema, params.name)
            .then((r) => {
    ...
    useEffect(() => {
        ...
        let active = true;
        cmd.getConstraints(connectionId, params.schema)
            .then((cs) => {
                if (!active) return;
                const names = new Set<string>();
                for (const c of cs.filter((x) => x.table === params.name)) {
                    const m = /FOREIGN KEY\s*\(([^)]+)\)/i.exec(
                        c.definition ?? "",
                    );
    ...
    const removeColumn = (i: number) =>
        patchColumns(action.columns.filter((_, j) => j !== i));
    ...
    const handleFkStaged = (result: {
        pairs: { localCol: string; refType: string }[];
        fk?: FkDefinition;
    }) => {
    ...
    const stage = async () => {
        ...
        try {
            ...
            if (mode === "edit" && action.old_columns !== undefined) {
                const live = await cmd.getTableColumns(
                    connectionId,
                    params.schema,
                    params.name,
                );
                const liveCols: TableFormColumn[] = live.map(
                    (c: ColumnInfo, i) => ({
                        rowId: `live-${i}`,
                        name: c.name,
                        type: c.data_type,
                ...
                if (!sameColumns(liveCols, action.old_columns)) {
                    setError(
                        "Table changed since you opened it — re-open to see current state",
                    );
                    return;
                }
            }
            ...
            <div className="flex items-center justify-between border-b border-border px-4 py-2">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        onClick={stage}
                        disabled={
                            !!error || !!refusal || staging || cols.length === 0
                        }
                ...
                {refusal && (
                    <div className="border-b border-border px-4 py-2">
                        <p className="text-xs text-amber-400">
                            Cannot reorder: {refusal}. Use the Query tab with
                            pg_dump for these tables.
                        </p>
                    </div>
                )}
                    ...
                    <>
                        ...
                        >
                            <div className="border-b border-border flex items-stretch w-full min-w-[760px]">
                                ...
                                <div className="w-8 shrink-0 border-r border-border px-3 py-1.5 flex items-center text-[11px] font-semibold text-text-muted uppercase tracking-wider">
                                    #
                                </div>
                                <div className="flex-1 min-w-48 border-r border-border px-3 py-1.5 flex items-center text-[11px] font-semibold text-text-muted uppercase tracking-wider">
                                    Name
                                </div>
                                <div className="flex-1 min-w-40 border-r border-border px-3 py-1.5 flex items-center text-[11px] font-semibold text-text-muted uppercase tracking-wider">
                                    Type
                                </div>
                                <div className="w-24 shrink-0 border-r border-border px-3 py-1.5 flex items-center text-[11px] font-semibold text-text-muted uppercase tracking-wider">
                                    Parameters
                                </div>
                                <div className="flex-1 min-w-44 border-r border-border px-3 py-1.5 flex items-center text-[11px] font-semibold text-text-muted uppercase tracking-wider">
                                    Default Value
                                </div>
                        ...
                        <RelationshipsSection
                            ...
                            onAddFk={() =>
                                setFkPanel({ open: true, column: null })
                            }
                            onRemoveCreateFk={removeCreateFk}
                            onEditCreateFk={editCreateFk}
                            onOpenPanel={(col) =>
                                setFkPanel({ open: true, column: col })
                            }
                        />
                        ...
                        <AnimatePresence>
                            {fkPanel?.open && (
                                <FkPanel
                                    ...
                                    mode={mode ?? "edit"}
                                    localColumns={(action.columns ?? []).map(
                                        (c) => ({
                                            name: c.name,
                                            data_type: c.type,
                                        }),
                      ...
                      >
                          <label className="flex items-center gap-2 px-2 py-1 text-xs text-text whitespace-nowrap cursor-pointer">
                              <input
                                  ...
                                  disabled={mode === "edit"}
                                  onChange={(e) =>
                                      setCell(index, "is_pk", e.target.checked)
                                  }
                ...
                <input
                    ...
                    onChange={(e) =>
                        setCell(
                            index,
                            "default",
                            e.target.checked ? (c.default ?? "") : null,
                        )
                    }
                    className="rounded border-border bg-surface text-accent focus:ring-accent"
                />
...
function OptionsSection({
    ...
    useEffect(() => {
        let active = true;
        cmd.getTablespaces(connectionId)
            .then((ts) => {
...
function parseFkSql(
    text: string,
): Omit<FkRow, "key" | "source" | "localTable"> {
    ...
    const localCols = local
        ? local[1].split(",").map(strip).filter(Boolean)
        : [];
...
function RelationshipsSection({
    ...
    useEffect(() => {
        let active = true;
        cmd.getConstraints(connectionId, schema)
            .then((cs) => {
                if (active)
                    setConstraints(
                        Array.isArray(cs)
                            ? cs.filter((c) => c.table === table)
                            : [],
                    );
            })
            .catch(() => {
    ...
    const fks = useMemo(
        () =>
            constraints.filter((c) =>
                (c.definition ?? "").toUpperCase().includes("FOREIGN"),
            ),
        [constraints],
    );
                    ...
                    <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-1.5">
                                ...
                                <div className="flex flex-row gap-1 items-center bg-text/10 px-2 py-1 rounded">
                                    <Table2
                                        size={12}
                            ...
                            <p className="font-mono text-[11px] text-text-muted mt-0.5 truncate">
                                ...
                                {row.localCols.join(", ")} → {row.refTable}.
                                {row.refCols.join(", ")}
